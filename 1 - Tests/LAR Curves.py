...
fins = [[0, 0.1], [0, 0.1], 1/10, 0.001]  # AR = 2
...
plt.legend(["AR = 2"], fontsize=legend_font_size)
...
AR_list = [1.25, 1.5, 1.75, 2]
AR_list_str = ["1.25", "1.5", "1.75", "2"]
legend_list = [""]*8
for i in range(len(AR_list)):
    legend_list[i] = "AR=" + AR_list_str[i]
for i in range(len(AR_list)):
    legend_list[i+4] = "AR=" + AR_list_str[i]
marker = ["v", "s", "v", "^"]
colors = ["C0", "C1", "C2", "C3"]
