...
class Fin:
    ...
    def _calculate_real_dimension(self):
        ...
        # sweep length
        self.Xt = self._dim[1][0] - self._dim[0][0]
        # b = wingspan
        self.b = self._dim[1][1] - self._dim[0][1]
    ...
    def _calculate_mac_xf_ar(self):
        ...
        if self.fin_attached is True:
            ...
            # plate of 0.2 h/b. Rounding down, the aspect ratio is increased
            # by 1-(r/2) being r the correction factor (r = 0.75). One must
            # remember that this end plate acts not to increase the AR/Lift
            ...
            # The formula checks at the extremes: r=1 (no end plate, AR = AR
            # of one fin alone (0.5 calculated AR)), r=0 (fin attached to the
            # body, AR doubles (i.e. stays the same as the one calculated
    ...
    def _calculate_force_application_point(self):
        self.CP_total = self._dim[0][0] + self.Xf
    ...
    def _correct_cna_diederich(self):
        ...
        COS_SBe = (((self.Beta)
                    / np.sqrt((1 - self.M**2 * np.cos(self.sb_angle)**2)))
...
class Rocket:
    ...
    def calculate_aero_coef(self,AoA,V0 = 10,rho = 1.225,mu = 1.784e-5,
                            M = 0.001, Actuator_angle = 0):
        if M < 0.001:
            self.M = 0.001
        else:
            self.M = M
