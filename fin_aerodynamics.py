...
import wind_tunnel_data
import fin_physical_properties
...
class Fin:
    ...
    def get_aero_coeff(self, Re, Re_crit, use_rocket_re=True):
        ...
        self.cp = self.acoeff.hca * self.pp.mac + self.pp._dim[0][0]
        return self.acoeff
...
class AerodynamicProperties:
    ...
    def get_aero_coeff(self, fin_flight_cond, Re, Re_crit, use_rocket_re):
        ...
        self.acoeff.set_cl_cd_cn_ca_hca_cd0(self._get_cl_cd_cn_ca_hca(fin_flight_cond), self.cd0)
        return self.acoeff
    ...
    def _get_cl_cd_cn_ca_hca(self, fin_flight_cond):
        ...
        self.hca = self._calculate_hca()
        return self.cl, self.cd, self.cn, self.ca, self.cm, self.hca
    ...
    def _calculate_cl_ular(self):
        ...
        delta_stall_angle = 4*DEG2RAD * (self.pp.transition_ar[0] / self.pp.aspect_ratio)
        if delta_stall_angle > 10*DEG2RAD:
            delta_stall_angle = 10*DEG2RAD
...
class AerodynamicCoefficients:
    ...
    def __init__(self):
        self.cl, self.cd, self.cn, self.ca, self.cm, self.hca, self.cd0 = 0, 0, 0, 0, 0, 0, 0

    def set_cl_cd_cn_ca_hca_cd0(self, a, b):
        self.cl, self.cd, self.cn, self.ca, self.cm, self.hca = a
        self.cd0 = b
