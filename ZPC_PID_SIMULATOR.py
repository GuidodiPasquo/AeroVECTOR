...
Angular velocity is not damped, so when tumbling the rocket will turn
way more than in real life.
If your angular velocities are small, the lack of damping doesn't
affect too much the flight, if the velocities aren't small, then the
damping in real life would make the real model more stable than in
the simulation (most times)
...
def run_3d():
    ...
    if toggle_3D is True:
        ...
        for i in range(len(theta_3D)-2):
            ...
            vect_cp=vp.vector(rocket_3d.pos.x+xa_radius[0],rocket_3d.pos.y+xa_radius[1],0)

            # Rotate rocket from the CG
            rocket_3d.rotate((theta_3D[i+1]-theta_3D[i]),axis=vp.vector(0,0,1),origin=vect_cg)
            ...
            if Camera_type=="Follow":
                scene.camera.pos = vp.vector(rocket_3d.pos.x+camera_shake[1]/50,rocket_3d.pos.y+1.2-camera_shake[0]/500,rocket_3d.pos.z-1)
                scene.camera.axis=vp.vector(rocket_3d.pos.x-scene.camera.pos.x , rocket_3d.pos.y-scene.camera.pos.y , rocket_3d.pos.z-scene.camera.pos.z)
            ...
            # Lateral camera, like if it was 2D
                ...
                scene.fov=fov*DEG2RAD
                scene.camera.pos = vp.vector(rocket_3d.pos.x+camera_shake[1]/50,rocket_3d.pos.y+0.0-camera_shake[0]/200,rocket_3d.pos.z-70*5)
                scene.camera.axis=vp.vector(rocket_3d.pos.x-scene.camera.pos.x , rocket_3d.pos.y-scene.camera.pos.y , rocket_3d.pos.z-scene.camera.pos.z)
