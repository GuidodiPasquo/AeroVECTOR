...
burnout_time=5 # Burn time of the motor, stops the simulation

Thrust_curve=False         #Thrust=(max_thrust/(burnout_time/7)*t) DURING 0<t<burnout_time/7
...
max_thrust=10           #If you use thrust curve set them to your motor value
average_thrust=10
t_launch = 1
...
CA0=0.43 #AXIAL FORCE COEFFICIENT at alpha=0
...
def update_servo():      
    ...
    asc11=0.
    asc12=1.
    asc21=-(-2624.5 * np.power(u_delta,3) + 9996.2 * np.power(u_delta,2) - 13195 * u_delta + 6616.2)  # Straights that modify the model of the sevo based on its current position and setpoint
    asc22=-(39.382 * np.power(u_delta,2) - 125.81 * u_delta + 124.56)
    ...
    # B Matrix
    bsc11=0
    bsc21=(-2624.5 * np.power(u_delta,3) + 9996.2 * np.power(u_delta,2) - 13195 * u_delta + 6616.2)
...
def update_parameters():
    ...
    q = 0.5 * rho * np.power((np.sqrt( np.power(V_loc_tot[0],2) + np.power(V_loc_tot[1],2) )),2) # Computes the dynamic pressure
...
def Nalpha_calculator(AoA):    
    ...
    if(AoA>=0 and AoA<np.pi/2):        
        Nalpha = k1 * np.power(AoA,3) + k2 * np.power(AoA,2) + k3 * AoA
        
    elif(AoA<0 and AoA>=-np.pi/2):
        Nalpha = -( k1 * np.power(abs(AoA),3) + k2 * np.power(abs(AoA),2) + k3 * abs(AoA))
        
    elif(AoA<-np.pi/2):        
        AoA_loc=-AoA-np.pi # Eg, -91º = -89º
        Nalpha = -( k1 * np.power(abs(AoA_loc),3) + k2 * np.power(abs(AoA_loc),2) + k3 * abs(AoA_loc))
        
    elif(AoA>=np.pi/2):
        AoA_loc=np.pi-(AoA)  # Eg, 91º = 89º
        Nalpha = k1 * np.power(AoA_loc,3) + k2 * np.power(AoA_loc,2) + k3 * AoA_loc 
    else:
        Nalpha = 0
...
def CA_calculator(AoA,CA0):
    ...
    CA = CA0 * 1.4 * np.power(np.sin(1.4*abs(AoA)+55*deg2rad),2)
...
#!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
########################################################################################################################## Software in the Loop Parameters 
...
force_reduction=3 #smaller or bigger arrows 1= one meter per newton, 5=0.2m/N
...
########################################################################################################################## PID GAINS
kp=5 #0.4  #Gains might be quite high to compensate for aerodynamic forces
ki=0 #0.1
kd=2*0.3 #0.136
k_all=1 
k_damping=0 #when possible use gyro feedback instead of derivative so as to not amplify noise

anti_windup=False #Prevents the integrator for integreting when the TVC is saturated
                 #it also limits the output of the PID so k_damping is more efective

torque_controller=False #Activates the torque controller
...
T_glob=0.001 #0.001   #T=Sample time of the simulation 
...
clock_dif=0.94 #clock_dif because the Arduino clock is slower than the PC one
...
while t<=Sim_duration:
    ...
    if time.perf_counter()/clock_dif > t0+T_glob:
        ...
        t_verif=time.perf_counter()/clock_dif-t0
    ...
    #!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
        ...
        setpoint_plot.append(t_verif*1000)
        theta_plot.append(theta)
        servo_plot.append(u_servos)
        
        test_plot.append(Position_global[0]*0)
    ...
    if serialArduino.inWaiting()>1:
        ...
        if read == "R":
            ...
            send = str(round(Q*rad2deg,6))+","+str(round(Accx/9.81,6))+","+str(round(Accz/9.81,6))+","+str(round(Position_global[0],2))+","                       
...
if toggle_3D==True:
    ...
    #Just creating the labels, only the position matters
    ...
    servo_label = vp.label(canvas=labels, pos=vp.vector(-60,1,0), text="Setpoint = %.2f"+str(setpoint_plot[i]*rad2deg), xoffset=0, zoffset=0, yoffset=0, space=30, height=30, border=0,font='sans',box=False,line=False , align ="left")
    V = vp.label(canvas=labels, pos=vp.vector(-60,-2,0), text="Velocity"+str(setpoint_plot[i]*rad2deg), xoffset=0, zoffset=0, yoffset=0, space=30, height=30, border=0,font='sans',box=False,line=False,align ="left")
    AoA_plot = vp.label(canvas=labels, pos=vp.vector(-60,-5,0), text="Velocity"+str(setpoint_plot[i]*rad2deg), xoffset=0, zoffset=0, yoffset=0, space=30, height=30, border=0,font='sans',box=False,line=False,align ="left")
    Altitude = vp.label(canvas=labels, pos=vp.vector(-60,-8,0), text="Velocity"+str(setpoint_plot[i]*rad2deg), xoffset=0, zoffset=0, yoffset=0, space=30, height=30, border=0,font='sans',box=False,line=False,align ="left")
    Time_label = vp.label(canvas=labels, pos=vp.vector(40,7,0), text="Time"+str(setpoint_plot[i]*rad2deg), xoffset=0, zoffset=0, yoffset=0, space=30, height=30, border=0,font='sans',box=False,line=False,align ="left")
    ...
    for i in range(len(theta_3D)-2):
        ...
        servo_label.text = "TVC deflection = " + str(round((servo_3D[i]-u_initial_offset)*rad2deg,2)) + u'\xb0'
...
plt.plot(t_plot,theta_plot,t_plot,setpoint_plot,t_plot,servo_plot,t_plot,test_plot)
